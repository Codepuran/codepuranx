...
- Apply the project naming rules to new code and edits.
- Review proposed names against the reference before writing them.
- Prefer the shortest name that stays clear under the project rules.
- Keep generated names consistent with nearby code and repo conventions.

## Output Style

- If asked to explain naming, summarize only the relevant rule(s).
- If asked to rename code, return the renamed identifiers and the reason only when needed.
